...
def test_committed_construction_snapshot_reflects_override():
    ...
    pre_score = next(
        g["goal_risk_score"] for g in snapshot_pre["goals"] if g["id"] == "goal_ski_cabin"
    )
    ...
    post_score = next(
        g["goal_risk_score"] for g in snapshot_post["goals"] if g["id"] == "goal_ski_cabin"
    )
    assert post_score == 1  # override-resolved
    # Goals without overrides remain unchanged in the snapshot.
    other_goals_pre = {
        g["id"]: g["goal_risk_score"] for g in snapshot_pre["goals"] if g["id"] != "goal_ski_cabin"
    }
    other_goals_post = {
        g["id"]: g["goal_risk_score"] for g in snapshot_post["goals"] if g["id"] != "goal_ski_cabin"
    }
...
def test_override_changing_effective_score_produces_new_run_signature():
    ...
    assert run_seed is not None, "load_synthetic_personas should auto-generate the initial run"
    ...
    ski_cabin_recs = run_after.link_recommendation_rows.filter(goal_external_id="goal_ski_cabin")
