...
**Last updated:** 2026-05-04 AM (sub-sessions #1+#2+#3 COMPLETE; backend + frontend wiring shipped; A6 deferred to sub-sessions #4-#5)
**Branch:** `feature/ux-rebuild` (cut from `main` per locked decision #9; ahead of origin by 8 commits as of HEAD `303e378`; user pushes Mon morning)
**HEAD:** `303e378` (test fix post-A5 auto-seed) — Tags: `v0.1.0-pilot` at `d2abfa1` + `v0.1.1-improved-intake` at intake-hardening commit; `v0.1.2-engine-display` deferred to sub-session #5 A6.10
