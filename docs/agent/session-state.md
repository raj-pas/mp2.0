...
**Last updated:** 2026-05-05 (post-tag gap-closure sub-session #3 EXTENDED+ — **2 CRITICAL pilot-blocking fixes** shipped mid-A6.1 USER MANUAL real-Chrome smoke. **Fix #1** (`870563b`): goal-risk-override→engine flow — override mechanism was theatre pre-fix because `committed_construction_snapshot` + `_goal_to_engine` ignored `GoalRiskOverride` rows; new `effective_goal_risk_score` helper resolves before engine input. Verified live: pre-save sig 008F5F87 + Balanced-growth blend → post-save sig CD78E7C1 + Cautious blend (95.4% Savings ideal) on Ski cabin's 8yr horizon. **Fix #2** (`78c635a`): CMA Workbench save handlers (assumptions + correlations) sent partial payloads but backend requires full replacement; analysts couldn't publish new CMAs. Verified live: analyst saved + published; advisor saw stale chip on Sandra/Mike household route; Regenerate cycled to fresh run `6FCD19B6` with new CMA's expected_return values flowing into the engine output. A6.1 Steps 1-8 verified in real Chrome; Step 9 (HASH_MISMATCH integrity overlay; optional, requires Django shell) + Step 10 (console clean; implicit) + A7 close-out remaining.)
**Branch:** `feature/ux-rebuild` (16 commits past tag `v0.1.2-engine-display`)
**HEAD:** `78c635a` (CMA save fix) past tag `v0.1.2-engine-display` at `e5cd859`. Tags unchanged: v0.1.0-pilot + v0.1.1-improved-intake + v0.1.2-engine-display. NEW tag `v0.1.3-engine-display-polish` cuts at A7 close-out per §3.22.
