...
**Last updated:** 2026-05-05 (post-tag gap-closure sub-session #3 EXTENDED+ — **CRITICAL FIX** for goal-risk-override→engine flow shipped at HEAD `870563b` mid-A6.1 USER MANUAL real-Chrome smoke; the override mechanism was theatre pre-fix because `committed_construction_snapshot` + `_goal_to_engine` ignored `GoalRiskOverride` rows. Fix verified end-to-end in real Chrome on Sandra/Mike's `goal_ski_cabin` (8yr horizon, no horizon-cap collapse): pre-save sig 008F5F87 + Balanced-growth blend → post-save sig CD78E7C1 + Cautious blend (95.4% Savings ideal). A6.1 Steps 1-7 verified in real Chrome; Steps 8 (CMA republish stale overlay), 9 (HASH_MISMATCH integrity), 10 (console clean) + A7 close-out remaining.)
**Branch:** `feature/ux-rebuild` (15 commits past tag `v0.1.2-engine-display`)
**HEAD:** `870563b` (engine_adapter fix) past tag `v0.1.2-engine-display` at `e5cd859`. Tags unchanged: v0.1.0-pilot + v0.1.1-improved-intake + v0.1.2-engine-display. NEW tag `v0.1.3-engine-display-polish` cuts at A7 close-out per §3.22.
