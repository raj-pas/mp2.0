...
| 10 | Backend Docker image stale w.r.t. R0 deps (`django-csp`) | `Dockerfile` baked venv at `/opt/mp20-venv` | **Action item before CI / pilot.** Backend container image was built before R0 added `django-csp` to `pyproject.toml`; current image is missing the module so Django won't start in compose. A clean `docker compose build backend` resolves it (Docker pypi was timing out at the time of the smoke; retry when network is healthy). Host-mode dev works via the new Vite proxy in the meantime. |
