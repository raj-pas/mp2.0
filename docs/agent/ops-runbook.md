...
## §2. Portfolio Run Integrity Alert (post-tag v0.1.3-engine-display-polish)

`portfolio_run_integrity_alert` AuditEvent fires when
`HouseholdDetailSerializer` observes a `PortfolioRun` with status
`hash_mismatch` (computed by `_portfolio_run_status` when a
`HASH_MISMATCH` PortfolioRunEvent is present on the run). Per locked
§3.5 this audit emission gates ENGINEERING attention; the advisor sees
an `IntegrityAlertOverlay` on the Goal route (no Regenerate button —
engineering action required).

### §2.1 Detection

```sql
-- Recent integrity alerts (last hour)
SELECT
    actor                          AS observed_by,
    entity_id                      AS run_external_id,
    metadata->>'household_id'      AS household_id,
    metadata->>'engine_version'    AS engine_version,
    created_at
FROM audit_auditevent
WHERE action = 'portfolio_run_integrity_alert'
  AND created_at > now() - interval '1 hour'
ORDER BY created_at DESC;
```

```sql
-- Frequency by run (catches repeated occurrences across advisors)
SELECT
    entity_id                      AS run_external_id,
    metadata->>'household_id'      AS household_id,
    COUNT(DISTINCT actor)          AS distinct_advisors_observing,
    MIN(created_at)                AS first_observed,
    MAX(created_at)                AS most_recent
FROM audit_auditevent
WHERE action = 'portfolio_run_integrity_alert'
  AND created_at > now() - interval '24 hours'
GROUP BY entity_id, metadata->>'household_id'
ORDER BY distinct_advisors_observing DESC, most_recent DESC;
```

### §2.2 Decision tree

#### Branch 1: Recent migration deployed (within 24h of first alert)

Most likely cause: schema or hash-computation change in `engine/` that
shifted `output_hash` for an existing run without invalidating the
PortfolioRunEvent.HASH_MISMATCH record.

- Verify via `git log --since='24 hours ago' engine/`
- If a hash-relevant change shipped: confirm intentional; for affected
  runs run a force-regenerate to produce a fresh run with current hashes
- Add a migration to clean up the stale `HASH_MISMATCH` events if the
  hash drift was migration-driven (NOT data corruption)

#### Branch 2: Engine version drift between PortfolioRun creation and observation

If `metadata.engine_version` differs from the current `engine.__version__`,
the run was generated by an older engine release. Re-running the engine
optimizer on the same household input should produce a consistent hash
under the current version.

- Force-regenerate via API:
  `POST /api/clients/<household_id>/generate-portfolio/`
- For real-PII households requiring intervention: confirm with the
  affected advisor before regenerating to avoid disrupting in-flight work

#### Branch 3: Genuine DB corruption (no recent migration; same engine version)

Most severe case: PortfolioRun row's `output_hash` field doesn't match
its `output` JSONB. Investigate:

```sql
-- Pull the suspect run for inspection (manual SHA-256 verification)
SELECT external_id, output_hash, engine_version, created_at
FROM api_portfoliorun
WHERE external_id = '<run_external_id>';
```

Then in a Django shell:
...
run = models.PortfolioRun.objects.get(external_id='<run_external_id>')
recomputed = sha256(json.dumps(run.output, sort_keys=True).encode()).hexdigest()
print(f'stored: {run.output_hash}')
print(f'recomputed: {recomputed}')
print(f'match: {run.output_hash == recomputed}')
```

If `recomputed != stored` → DB corruption. Escalate to engineering
on-call; do NOT attempt to "fix" by overwriting the hash.

### §2.3 Escalation criteria

- **>1 distinct advisor observes the same run within 24h** → page on-call
  engineering; may indicate a broader migration or data-pipeline issue
- **Same household alerts for >1 distinct run within 24h** → investigate
  household-specific data integrity (audit tables, related rows)
- **>5 alerts/hour across the system** → halt all auto-trigger paths via
  `MP20_ENGINE_ENABLED=False`; investigate before re-enabling

### §2.4 Recovery procedure

1. Identify the affected run(s) via §2.1 query
2. Determine root cause via §2.2 decision tree
3. If safe to regenerate (Branch 2 most common): force-regenerate via
   `POST /api/clients/<household_id>/generate-portfolio/` (or via
   Django shell calling `_trigger_portfolio_generation`)
4. Verify post-regenerate: `_portfolio_run_status(new_run)` returns
   `current`, no new `HASH_MISMATCH` event recorded
5. Document the incident in this runbook (append-only log file
   `docs/agent/integrity-alert-log.md` if alerts become recurring)

### §2.5 Audit emission contract (tested)

The dedup invariant is locked via property tests at
`web/api/tests/test_status_audit_invariants.py`:

- One row per `(run_external_id, advisor_email)` pair, regardless of GET
  count (verified in `test_integrity_audit_dedup_invariant`)
- Metadata never contains `str(exc)` patterns; no SIN / long-digit
  patterns leak (verified in `test_audit_metadata_has_no_pii_patterns`)
- Action string always canonical (`portfolio_run_integrity_alert`); never
  derived from exception class names (verified in
  `test_status_observation_emits_only_canonical_actions`)
