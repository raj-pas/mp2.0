...
Sub-session #2 of the post-tag gap-closure plan (`~/.claude/plans/i-want-you-to-jolly-beacon.md`) closes 2 of the remaining 4 functional/UX gaps from the original 2,786-line Engine→UI plan. Sub-session #1 (Phases A0+A1+A2 at `c5a7e02`) had pinned the backend stale-lifecycle contract (5 status semantics + integrity-alert audit emission + Hypothesis invariants + backwards-compat) and shipped the shared `<SourcePill>` consumed by GoalAllocationSection + MovesPanel. Sub-session #2 extends the SourcePill pattern to OptimizerOutputWidget (Phase A3) AND introduces the bespoke modal-style overlays for the 4 stale-status variants per locked §3.2 (Phase A4). Boot state matched expectations exactly: 872 backend pytest + 195 Vitest in 22 files; engine probe confirmed Sandra/Mike at `status=current` with both `link_recommendations[]` and `goal_rollups[]` populated (the engine path is exercisable on real fixtures, not just synthetic mocks). All static gates clean at entry. The boot consumed only the dossier + plan + last handoff entry; no exploratory drift.

**Phase A3 — `OptimizerOutputWidget` engine-first refactor (commit `c212793`).** Mirrors the A2 GoalAllocationSection pattern: `findGoalLinkRecommendations(household, goal.id)` + `useOptimizerOutput(householdId, goalId)` always called (no conditional hooks); SourcePill flips between engine / calibration / calibration_drag per locked §3.1 + §3.7. Engine path computes dollar-weighted improvement_pct: `idealReturn = Σ(l.expected_return × l.allocated_amount) / Σ l.allocated_amount`; `currentReturn = Σ((l.current_comparison?.expected_return ?? l.expected_return) × l.allocated_amount) / Σ l.allocated_amount`; `improvement_pct = (idealReturn − currentReturn) × 100`. The `* 100` is load-bearing — backend's `improvement_pct` is pct-scale (web/api/preview_views.py:476), not 0..1; first test run caught this (formatPct(0.05, 1) returned "0.1%" instead of "5.0%"). The `current_comparison.expected_return` null-guard falls back to `link.expected_return` so the comparison degenerates to "no improvement" rather than NaN-ing the blend. effective_descriptor + ideal_low + current_low remain calibration-sourced (no clean engine analogue for the dollar bands; descriptor reflects the same canon 1-5 scale). 7 new Vitest tests covering: engine path with mock link recs / null current_comparison fallback / dollar-weighted multi-link blend (verified 2.75pp computed → "2.8%" formatted) / calibration fallback when no engine links / empty links array no-throw / drag pill flip / StrictMode (locked #64). Total Vitest: 195 → 202 in 23 files.

**Phase A4 — Stale-state UX with 4 status variants + IntegrityAlertOverlay (commit `8350090`).** Two NEW components plus modifications to RecommendationBanner + HouseholdPortfolioPanel + GoalRoute + i18n. `<StaleRunOverlay status onRegenerate isPending>` (~110 LoC): `role="alertdialog"` + `aria-modal=true` + `aria-labelledby` + `aria-describedby`; bespoke focus model per locked #68 mirroring DocDetailPanel.tsx:56-67 — captures `previousFocusRef = document.activeElement` on mount; auto-focuses Regenerate; restores focus on unmount; Esc does NOT dismiss (informational, not modal — advisor cannot bypass; must regenerate); blurs active element on Esc so Tab cycle doesn't trap behind overlay; Tab focus-trapped to Regenerate (only focusable element). Three status variants (invalidated / superseded share copy "Stale: regenerate to refresh"; declined uses "Run was declined; regenerate to retry"). `<IntegrityAlertOverlay runSignature>` (~50 LoC): `role="alert"` (not alertdialog — no buttons inside); no focus management; Esc does NOT dismiss; renders signature + ops-runbook §2 reference for engineer triage. Backend audit emission for hash_mismatch already shipped in A1 (web/api/serializers.py:298-340); this overlay is purely UI. RecommendationBanner expanded from 3 → 5 visual states: stale chip (warning border + role=status + aria-live=polite + Regenerate) for invalidated/superseded/declined; integrity chip (danger border + role=alert + NO Regenerate) for hash_mismatch. HouseholdPortfolioPanel mirrors the same chip pattern per locked #19; metrics + top-funds remain visible but muted via opacity-40+pointer-events-none when status is non-current. GoalRoute wraps GoalAllocationSection + AdvisorSummaryPanel + OptimizerOutputWidget + MovesPanel in a relative div with overlay positioned absolutely; muted+aria-hidden when status non-current; `useGeneratePortfolio` hook lifted above early returns to satisfy Rules of Hooks (caught by lint, not typecheck). i18n keys added under `routes.goal.*`: stale_overlay_title/_body/_regenerate, declined_overlay_title/_body, integrity_overlay_title/_body/_run_ref, stale_chip_label, integrity_chip_label; under `routes.household.*`: stale_chip_label, integrity_chip_label. Tests: NEW StaleRunOverlay.test.tsx (10 — ARIA + 3 focus model tests + Esc no-dismiss + Tab focus-trap + Regenerate behavior + 3 copy variants); NEW IntegrityAlertOverlay.test.tsx (7 — role=alert + no-Regenerate + signature reference + Esc no-dismiss + ARIA + StrictMode + null signature handling); RecommendationBanner.test.tsx +5 (3 stale variants + integrity + click-Regenerate); HouseholdPortfolioPanel.test.tsx +4 (2 stale variants + integrity + click-Regenerate). Total Vitest: 202 → 228 in 25 files.

**Followup `chore` commit `64ab152`:** Cleaned up two artifacts (`.claude/scheduled_tasks.lock` + `test-results/.last-run.json`) that landed in the A4 commit via an unintended `git add -A`. Per CLAUDE.md preference, used a follow-up commit (not `--amend`) and extended `.gitignore` to cover root-level `.claude/` + `test-results/` so they stay untracked going forward. Going forward in A5+ I will stage explicit file lists. Caught and surfaced via AskUserQuestion before continuing.

**Gates at HEAD `64ab152` (sub-session #2 close-out):**
- Backend pytest: **872 passed + 2 skipped** (unchanged — A3+A4 are frontend-only; backend baseline preserved)
- Frontend Vitest: **228 passed in 25 files** (was 195 in 22 baseline; +33 net new across A3+A4)
- typecheck/lint/build clean at all 4 commits (one Rules-of-Hooks lint catch in A4 was fixed before commit by lifting `useGeneratePortfolio` above early returns)
- Bundle: **269.41 kB gzipped** (was 267.55 baseline; +1.86 kB; under 290 kB locked #85; ~21 kB headroom)
- Static guards (vocab CI / PII grep / OpenAPI codegen): all OK at every commit gate
- Theme tokens (accent-2 / warning / danger / muted) verified in tailwind.config.ts per locked §3.10
- Engine probe at boot: Sandra/Mike `status=current`, `has_links=true`, `has_rollup=true` — engine path exercisable

**What's next — sub-session #3 (Phase A5 + A5.5 + A6 + A7).** Estimated 5-7 hr. Boot via THIS log entry + `docs/agent/post-tag-gap-closure-starter-prompt.md` (lifecycle: deleted at A7) + plan §A5/A5.5/A6/A7 sections. Sub-session #3 ships: A5 (demo script update + axe coverage on Goal+Household routes + visual baselines for engine pills + 6 cross-browser cells per §3.15); A5.5 (NEW `frontend/e2e/regression-coverage.spec.ts` with 15 automated browser tests for pre-existing flows per §3.20 — HEAVIEST phase, ~3-4 hr); A6 (USER MANUAL real-Chrome smoke per locked #100 + dress rehearsal per §3.25); A7 (2 subagent dispatches per §3.13 + 90% coverage gate per §3.14 + tag `v0.1.3-engine-display-polish` per §3.22 + delete starter prompt + close-out commit). The plan's halt-and-flush gate at end of A4 calls for user manual smoke (publish CMA → see overlay → click Regenerate → verify dismissal; insert HASH_MISMATCH event → see integrity overlay → confirm NO Regenerate + audit emission); deferring this to A6 dress rehearsal where it's already in scope, since both A5 + A5.5 work is data-shape-agnostic and the engine probe at boot already confirmed the surface renders correctly with current Sandra/Mike data. **Cumulative test bar at HEAD post-A4:** 872 backend pytest + 228 Vitest in 25 files + 13 foundation e2e + 32 visual-verification + 24 cross-browser + 4 pilot-features (axe pre-extension) = 1173 tests passing across all suites. Sub-session #3 will add ~+30 chromium Playwright (regression suite + new visual baselines + axe coverage + cross-browser extension) for a final cumulative target of ~1200+ tests at A7.
